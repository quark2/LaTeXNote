...
Remind that in the main lemma of Engel's theorem, for $\rho(\lie{i}) \trianglelefteq \rho(\lie{g})$ consisting of nilpotent endomorphisms, $\{v \in V \SBar \rho(X)v = 0 \textrm{ for all $\rho(X) \in \rho(\lie{i})$}\}$ is a non-zero proper submodule of $V$.
...
The next is supposing that there is  $c' \in k$ which is not an integer or a half-integer or a negative integer smaller than -1 such that $\lie{g}_{c\alpha} \ne 0$.
...
When $c'$ is not an integer nor a half-integer, since $|\Phi| < \infty$, there must be such $m$.
...
So we ruled out all $c\alpha$ where $c$ is an integer other than $\pm 1$ and non-integer without half-integers.
Now we shall check the cases for half-integers.
If $\lie{g}_{\frac{1}{2} \alpha} \ne 0$, then by the previous result $\lie{g}_{2 \cdot \frac{1}{2} \alpha} = \lie{g}_\alpha = 0$, a contradiction.
Thus $\lie{g}_{\frac{1}{2} \alpha} = 0$.
Now the remaining is similar to the above; suppose that there is a half-integer $c'$ with $\lie{g}_{c'\alpha} \ne 0$, then we can find another half-integer $c \ge c'$ where $\lie{g}_{c\alpha} \ne 0$ and $(\ad{E_\alpha}) \lie{g}_{c\alpha} = 0$, but $c$ must be smaller than $-\frac{1}{2}$.
Then, the highest weight of the irreducible $\lie{s}_\alpha$-submodule containing $\lie{g}_{c\alpha}$ is $2c$, which is negative, a contradiction.
Therefore, no any multiplicity of $\alpha$ by a half-integer cannot be in $\Phi$.
...
(B2) All $\alpha \in \Phi$ is a nonnegative or nonpositive $\InZ$-linear combination of $\Delta$.
...
Since $\Phi$ is finite, $E - \bigcup_{\alpha \in \Phi} P_\alpha \ne \emptyset$.
We call an element in this set \textbf{regular}.
By definition, $(\gamma, \alpha) \ne 0$ for all $\alpha \in \Phi$ and regular $\gamma$.
...
But it is direct from the result of the previous page; if not, that is, $(\alpha, \beta) > 0$, then $\alpha - \beta \in \Phi$.
It seems a contradiction to (B2), but not enough since there is a possibility to be written as a form indicated in (B2).
Instead, suppose that $\alpha - \beta \in \Phi^+(\gamma)$.
Then, $\alpha = (\alpha - \beta) + \beta$, so $\alpha$ is decomposable, a contradiction.
If we suppose that $\alpha - \beta \in \Phi^-(\gamma)$, then $\beta - \alpha \in \Phi^+(\gamma)$, so we can say that $\beta$ is decomposable, a contradiction.
Therefore, $\alpha - \beta$ cannot be in $\Phi$, so $(\alpha, \beta) \le 0$.
...
Then $\Delta(\gamma)$ is a base of $\Phi$.
...
Now we define \textbf{an ordering in $E$}: for $\lambda , \mu \in E$ we denote $\lambda \succ \mu$ when $\lambda - \mu = \sum_{\alpha \in \Delta} a_\alpha \alpha$ for $0 \le a_\alpha \in \InZ$.
Let $\lambda \in E$, and let M = $\{\sigma(\lambda) \SBar \sigma(\lambda) \succ \lambda, \sigma \in W\}$ and $\sigma \in W$ be such that $\sigma(\lambda)$ is maximal of $M$ in the ordering.
...
We choose and fix $\alpha \in \Delta_2$.
Since $\beta - \alpha$ is not a root and also $\beta + \alpha$ is not because of the maximality of $\beta$, we have that $(\beta, \alpha) = 0$.
Oh the other hand, since all of $a_i$ corresponding to each of $\Delta_1$ is positive and $(\alpha', \alpha) \le 0$ for every $\alpha' \in \Delta_1$, the fact that $(\beta, \alpha) = 0$ implies that $(\alpha', \alpha) = 0$ for every $\alpha' \in \Delta_1$, a contradiction to the assumed irreducibility.
...
Now we assume that we have two maximal roots $\beta$ and $\beta'$, and now we have an interest of $(\beta, \beta')$.
Since $\beta' - \alpha_i$ cannot be a root for every $i$ because of the maximality, we have that $(\alpha_i, \beta') \ge 0$, especially at least one of them must be positive.
On the other hand, we have shown that $\beta = \sum_i a_i \alpha_i$ with all positive $a_i$.
Thus, $(\beta, \beta') = \sum_i a_i (\alpha_i, \beta') > 0$.
We were looking for this because this inequality implies that $\beta - \beta'$ and $\beta' - \beta$ are roots if $\beta \ne \beta'$.
Then, since one of them is a positive root, we have that $\beta \succ \beta'$ or $\beta' \succ \beta$, but it is contradict to the maximality of $\beta$ and $\beta'$.
...
We consider the two sided ideal $I_L$ which is generated by all $X \otimes Y - Y \otimes X - [X, Y]$ for $X, Y \in \lie{g}$.
Let $U(\lie{g}) = T(\lie{g}) / I_L$.
...
We then consider the induced map $\phi : T(\lie{g})/I_L \to \mathcal{A}$ from $\phi_0$.
Obviously, this is the homomorphism which we seek and it is unique.
Therefore, $U(\lie{g})$ is the universal enveloping algebra.

To go further, we need some more configuration.
First, we denote the canonical projection $T(\lie{g}) \to U(\lie{g}) = T(\lie{g})/I_L$ by $\pi$, and we choose a basis $V_i$ of $\lie{g}$.
We denote the two-side ideal generated by $A \otimes B - B \otimes A$ for every $A, B \in \lie{g}$ by $I_S$, and $T(\lie{g})/I_S$ by $\mathcal{S}$.
Note that it is isomorphic to \textit{the subalgebra of symmetric tensors} $\tilde{\mathcal{S}}(\lie{g})$ ($\tilde{\mathcal{S}}^m(\lie{g}) = \tilde{\mathcal{S}}(\lie{g}) \cap T^m(\lie{g})$), which is spanned by symmetric tensors $\frac{1}{m!} \sum_{\sigma \in S_m} ( V_{i_{\sigma(1)}} \otimes V_{i_{\sigma(2)}} \otimes \cdots \otimes V_{i_{\sigma(m)}} )$ (so that $( \tilde{\mathcal{S}}(\lie{g}) + I_S ) / I_S = \mathcal{S}(\lie{g})$).
On the other hand, we have a \textit{filtration} $T_m(\lie{g}) = \bigoplus_{i = 0}^m T^m(\lie{g})$ or $T^m$.
It can be applied to $U(\lie{g})$; we denote $\pi(T_m(\lie{g}))$ by $U_m(\lie{g})$ or $U_m$.
Now we denote $U_m(\lie{g}) / U_{m - 1}(\lie{g})$ by $G^m(\lie{g})$ or $G^m$ (where $U_{-1} = 0$), and $\bigoplus_{m = 0}^\infty G^m(\lie{g})$ by $\mathcal{G}(\lie{g})$ or $\mathcal{G}$.
Since $U_r U_s \subseteq U_{r + s}$ for every $r$ and $s$, we have a natural product rule on $\mathcal{G}$, which satisfies $G_r G_s \subseteq G_{r + s}$.
We combine the compositions of canonical maps $T^m \to U_m \to G^m$ to make a linear map $\pi_G : T(\lie{g}) \to \mathcal{G}(\lie{g})$, where the kernel is denoted by $I_G$.
It is obvious that for any $A \in T^r(\lie{g})$ and $B \in T^s(\lie{g})$ we have that $\pi_G(A) \pi_G(B) = \pi_G(AB)$, which implies that $\pi_G$ is an algebra homomorphism.
Also, for $X, Y \in \lie{g}$, since $X \otimes Y - Y \otimes X \in T^2$ is sent to $[X, Y]$ in $U_1(\lie{g})$, we have that $\pi_G(X \otimes Y - Y \otimes X) = 0 \in U_2(\lie{g}) / U_1(\lie{g})$, which implies that $I_S \le I_G$.
From this, we have an algebra homomorphism $\omega : \mathcal{S}(\lie{g}) \to \mathcal{G}(\lie{g})$.

By the definition, $\omega$ is obviously surjective.
But that is not all; \textit{$\omega : \mathcal{S}(\lie{g}) \to \mathcal{G}(\lie{g})$ is an algebra isomorphism}.
We call this theorem \textbf{Poincar\'e-Birkhoff-Witt theorem}, or in short, \textbf{PBW theorem}.
In the next page, we introduce its proof and its important corollaries.

\newpage

\textbf{Poincar\'e-Birkhoff-Witt theorem}

Now, we shall prove a central lemma of the PBW theorem; \textit{for each $m \in \NaN$ and for any $i, j \in \mathcal{I}_b$, there uniquely exsits a linear map $f_m : \lie{g} \otimes \mathcal{S}_m \to \mathcal{S}$ such that (SA$_m$) $f_m(V_i \otimes t^I) - t_i t^I \in \mathcal{S}_{r}$ for $r \le m$ and $l(I) = r$, especially (SA'$_m$) it is 0 if $l(I) = m$ and $i \le I$, and (SB$_m$) $f_m(V_i \otimes f_m(V_j \otimes x)) = f_m(V_j \otimes f_m(V_i \otimes x)) + f_m([V_i, V_j \otimes x])$ for every $x \in \mathcal{S}_{m - 1}$.}
\textit{Moreover, $f_m|_{\lie{g} \otimes \mathcal{S}_{m - 1}} = f_{m - 1}$.}

The case for $m = 0$ is obvious.
Assume that for $m > 0$ we already have $f_{m - 1}$.
It is enough to investigate what $\zeta_{i, I} = f_m(V_i \otimes t^I)$ should be for each $i$ and $I$.
By (SA$_m$), $\zeta_{i, I}$ should be $f_{m - 1}(V_i \otimes t^I)$ for $I$ with $l(I) < m$, and $\zeta_{i, I} = t_i t^I$ if $l(I) = m$ and $i \le I$.
To look at the case with $l(I) = m$ but $i \not\le I$, we denote $I = (i_1, i_2, i_3, \cdots)$, and $I' = (i_2, i_3, \cdots)$.
Note that $t^I = t_{i_1} t^{I'} = f_{m - 1}(V_{i_1} \otimes t^{I'}) = f_m(V_{i_1} \otimes t^{I'})$.
Then the requirement yields that $f_m(V_i \otimes t^I) = f_m(V_i \otimes f_m(V_{i_1} \otimes t^{I'})) = f_m(V_{i_1} \otimes f_m(V_i \otimes t^{I'})) + f_m([V_i, V_{i_1}] \otimes t^{I'})$ by (SB$_m$), which can be written as $t_{i_1} t_i t^{I'} + f_{m - 1}(V_{i_1} \otimes Y) + f_{m - 1}([V_i, V_{i_1}] \otimes t^{I'})$, where $Y = f_{m - 1}(V_i \otimes t^{I'}) - t_i t^{I'} \in \mathcal{S}_{m - 1}$ (note that $i \le (i_1, I')$), which can be $\zeta_{i, I}$.
Now, one can have a (unique) linear map $f_m : \lie{g} \otimes \mathcal{S}_m$ which sends $V_i \otimes t^I$ to $\zeta_{i, I}$ as defined before, and it holds (SA$_m$) and (SB$_m$).

With this lemma, one can combine these $f_m$ to make a linear map $f : \lie{g} \otimes \mathcal{S} \to \mathcal{S}$ satisfying (SA$_m$), (SB$_m$).
This linear map guarrantees the existence of a representation $\rho : \lie{g} \to \lie{gl}(\mathcal{S})$ such that $\rho(V_i) t^I - t_i t^I \in \mathcal{S}_{r - 1}$ for $r \le l(I)$, especially it is 0 if $i \le I$.
Then we have the canonical extension of $\rho$, an algebra homomorphism $\rho : T \to L(\mathcal{S})$ with $I_L \subseteq \ker{\rho}$.
Now, we consider $A \in T_m \cap I_L$, and $\rho(A)$.
Of course, $\rho(A) = 0$.
Meanwhile, the highest term of $\rho(A) \cdot 1$, which is 0, can be written as $\sum_{I \in \mathcal{I}, l(I) = m} a_I t^I$ for some scalars $a_I$ if the highest homogeneous part of $A$ is written as $\sum_{I \in \mathcal{I}, l(I) = m} a_I U^I$.
It implies that the linear combination is in $I_S$.
Thus, we have that \textit{for every $m$, the $m$-homogeneous part of any element in $T_m \cap I_L$ is in $I_S$}.
Now, the PBW theorem can be proved.
As known, the injectivity of $\omega$ is remaining, which can be done if one shows that for every $A \in T^m$ with $A + I_L \in U_{m - 1}$, is in $I_S$.
Note that by the surjectivity of $T_{m - 1} \to U_{m - 1}$ we have $A' \in T_{m - 1}$ such that $A + I_L = A' + I_L$ so that $A - A' \in T_m \cap I_L$.
By the last lemma, the $m$-homogeneous part of it, which is equal to $A$, must be in $I_S$, which is desired.

There are some important corollaries.
The first corollary: \textit{Let $W \le T^m$, where $\pi_S : T^m \to \mathcal{S}^m$ sends $W$ isomorphically, then $\pi(W) \cap U_{m - 1} = 0$ (in $U_m$).}
Note that $\pi_G \circ \pi|_{T^m} = \omega \circ \pi_S|_{T^m}$.
Since $\omega$ and $\pi_S|_W$ are isomorphic, $\pi_G(\pi(W))$ is isomorphic to $W$, which implies that $\pi(W) \cap U_{m - 1} = \pi(W) \cap \ker{\pi_G|_{U_m}} = 0$.
Because the symmetric tensors are sent isomorphically to $\mathcal{S}$, now we have that \textit{the subalgebra of the symmetric tensors in $T$ is complement to $I_L$.}
Also, one can apply the first corollary to the subspace $W$ spanned by all $V^I \in T^m$ for ordered $I$, where $W$ is also sent by $\pi_S$ isomorphically.
Thus $\pi(V^I)$ is not in $U_{m - 1}$, and even $\pi_G(\pi(W)) = G^m$.
This implies that the set of all $\pi(V^I)$, or \textit{the set of $V^I \in U^m$ for all $m$ and for all $I$ forms a basis of $U(\lie{g})$, which we call the PBW basis.}
...
Thus, we obtain the desired homomorphism $\phi : \mathcal{F}_L(\mathcal{X}) \to \lie{g} (\subseteq U(\lie{g}))$.
The uniqueness of the homomorphism requires a fact that $U(\mathcal{F}_L(\mathcal{X})) \cong T(V_\mathcal{X})$ with the inclusion map $\iota_F$ of $\mathcal{F}_L(\mathcal{X})$ into $T(V_\mathcal{X})$ as the canonical embedding, which will be proved in the below.
Now assume that there is a Lie algebra homomorphism $\psi : \mathcal{F}_L(\mathcal{X}) \to \lie{g}$ with $\psi \circ \iota = f$.
Then by the assumption there is an associative algebra homomorphism $\tilde{\psi} : T(V_\mathcal{X}) \to U(\lie{g})$ such that $\tilde{\psi} \circ \iota_F = \psi$.
(For a convenience we identify $\lie{g}$ with that in $U(\lie{g})$, a subalgebra of $U(\lie{g})_L$.)
But then it also satisfies that $\tilde{\psi} \circ \iota = \tilde{\psi} \circ \iota_F \circ \iota = \psi \circ \iota = f$.
Thus $\tilde{\psi} = \phi_1$, which implies that $\psi = \phi$.
Therefore, $\mathcal{F}_L(\mathcal{X})$ is the free Lie algebra.
The uniqueness of free Lie algebra (up to isomorphism) is then obvious.

It is remaining to show that $U(\mathcal{F}_L(\mathcal{X})) \cong T(V_\mathcal{X})$.
For this it is enough to find that for any homomorphism $f: \mathcal{F}_L(\mathcal{X}) \to \mathcal{A}_L$ for any associative algebra $\mathcal{A}$ there is a unique homomorphism $\phi: T(V_\mathcal{X}) \to \mathcal{A}$ such that $\phi \circ \iota_F = f$.
For such $f$, we have a map $\iota \circ f: \mathcal{X} \to \mathcal{A}$.
Then there is a (unique) homomorphism $\phi: T(V_\mathcal{X}) \to \mathcal{A}$ such that $\phi \circ \iota = f \circ \iota$, or $\phi(Y) = f(Y)$ for every $Y \in \iota(\mathcal{X})$.
Since $\iota(\mathcal{X})$ generates $\mathcal{F}_L(\mathcal{X})$, $\phi \circ \iota_F$ must be equal to $f$.
Moreover, if there is a homomorphism $\psi: T(V_\mathcal{X}) \to \mathcal{A}$ such that $\psi \circ \iota_F = f$, then $\psi \circ \iota = f \circ \iota$, so that $\psi = \phi$ by the uniqueness of $\phi$.
Therefore, $U(\mathcal{F}_L(\mathcal{X})) \cong T(V_\mathcal{X})$.
...
There is another issue on the matching of simple Lie algebras and (irreducible) root systems; two simple Lie algebras having 'same' root system could be not isomorphic.
Fortunately, we can prove this theorem: \textit{Let $\lie{g}_1$, $\lie{g}_2$ be finite-dimensional simple Lie algebras and $\lie{h}_i$ a maximal toral subalgebra in $\lie{g}_i$ and $\Phi_i$ the root system relative to $\lie{h}_i$ for each $i$.}
\textit{And also let $\Delta_i$ be a basis of $\Phi_i$ for each $i$.}
\textit{If there is a bijection $\Delta_1 \ni \alpha_i \mapsto \alpha'_i \in \Delta_2$ such that $\langle \alpha'_i, \alpha'_j \rangle = \langle \alpha_i, \alpha_j \rangle$ for every $i, j$ (which means that $\Phi_1$ and $\Phi_2$ are isomorphic), then $\lie{g}_1$ and $\lie{g}_2$ are isomorphic.}
Now we choose $E_i, F_i, H_i \in \lie{g}_1$ corresponding to $\Delta_1$, and $E'_i, F'_i, H'_i \in \lie{g}_1$ corresponding to $\Delta_2$.
The aim of this section is to find an isomorphism of $\lie{g}_1$ onto $\lie{g}_2$ which sends $E_i, F_i, H_i$ to $E'_i, F'_i, H'_i$, respectively, which is unique since $E_i$'s and $F_i$'s generate $\lie{g}_1$.

To show this, we need to think of a Lie algebra $\lie{g}_1 \oplus \lie{g}_2$, and denote $E_i \oplus E'_i$, $F_i \oplus F'_i$, $H_i \oplus H'_i$ by $\overline{E_i}$, $\overline{F_i}$, $\overline{H_i}$, respectively.
Now by $\lie{g}$ we denote the subalgebra of $\lie{g}_1 \oplus \lie{g}_2$ generated by $\overline{E_i}$, $\overline{F_i}$, $\overline{H_i}$.
Note that we have canonical projections $\pi_i : \lie{g}_1 \oplus \lie{g}_2 \to \lie{g}_i$ for each $i$, and $\pi_i(\lie{g}) = \lie{g}_i$, since it is equal to the subalgebra of $\lie{g}_i$ generated by $E_i$, $F_i$, $H_i$, which is equal to $\lie{g}_i$.
Surprisingly, if $\lie{g}$ is proper, then $\pi_i|_\lie{g}$ are injective.
Suppose that $\pi_2|_\lie{g}$ is not injective.
It implies that there is $X \oplus 0 \in \lie{g}$ for a non-zero $X \in \lie{g}$.
By applying polynomials of $\ad{\lie{g}}$ to $X \oplus 0 \in \lie{g}$, one can find $\tilde{\lie{g}_1} \oplus 0 \le \lie{g}$.
But it is obvious that $\ad{E_i}$, $\ad{F_i}$, $\ad{H_i}$ leaves $\tilde{\lie{g}_1}$ invariant, which implies that $\tilde{\lie{g}_1}$ is a non-zero ideal of $\lie{g}_1$, so $\tilde{\lie{g}_1} = \lie{g}_1$.
Thus $\lie{g}_1 \oplus 0 \subseteq \lie{g}$.
From this, one can find $0 \ne Y \in \lie{g}_2$ such that $0 \oplus Y \in \lie{g}$.
In the same way, we have that $0 \oplus \lie{g}_2 \subseteq \lie{g}$, thus $\lie{g} = \lie{g}_1 \oplus \lie{g}_2$, a contradiction.
Therefore, $\pi_2|_\lie{g}$ must be injective, and in the same reason, $\pi_1|_\lie{g}$ must be also injective.
Then $\lie{g}$ is isomorphic to $\lie{g}_1$ and $\lie{g}_2$, where $\overline{E_i}$, $\overline{F_i}$, $\overline{H_i}$ are sent to $E_i$, $F_i$, $H_i$ respectively, or $E'_i$, $F'_i$, $H'_i$, respectively, which we desired.

Unfortunately, it is hard to show that $\lie{g}$ is proper directly.
Instead, we borrow an idea which will be used for the uniqueness of irreducible modules, by using a proper $\lie{g}$-submodule and the 'maximal vector'.
Let $\beta_1$ and $\beta_2$ be the (unique) maximal root of $\Phi_1$ and $\Phi_2$, respectively, and we choose non-zero $Z_i \in ( \lie{g}_i )_{\beta_i}$, and let $Z = Z_1 \oplus Z_2$.
Note that because $\Phi_1$ and $\Phi_2$ are isomorphic, $\langle \beta_2, \alpha'_i \rangle = \langle \beta_1, \alpha_i \rangle$ for every $i$, which implies that (1) $(\ad{\overline{H_i}})Z = \langle \beta_1, \alpha_i \rangle Z$, and also (2) $(\ad{\overline{E_i}})Z = 0$.
Now by $S$ we denote the set of $(\ad{\overline{F_{i_1}}}) (\ad{\overline{F_{i_2}}}) \cdots (\ad{\overline{F_{i_r}}}) \overline{Z}$ for any $r$ and $i_j$'s, and by $\lie{m}$ the subspace spanned by $S$.
Note that, in the same way used in the existence theorem, with using (1) and (2), one can show that every $\ad{\overline{E_i}}$, $\ad{\overline{F_i}}$, $\ad{\overline{H_i}}$ sends any in $S$ to $\lie{m}$.
Thus $\lie{m}$ is a $\lie{g}$-submodule.
Note that since $\dim{(\lie{m} \cap ( ( \lie{g}_1 )_{\beta_1} \oplus ( \lie{g}_2 )_{\beta_2} ))} = 1$ (see $S$), while $\dim{( ( \lie{g}_1 )_{\beta_1} \oplus ( \lie{g}_2 )_{\beta_2} )} = 2$, $\lie{m}$ is proper.
Also, because $(\ad{(F_i \oplus E'_i)})Z = [F_i, Z_1] \oplus 0$, which is not zero for some $i$, in the same way for the proof of injectivity of $\pi_2|_\lie{g}$ we have that the action of $\ad{(\lie{g}_1 \oplus \lie{g}_2)}$ generates whole $\lie{g}_1 \oplus \lie{g}_2$ from $\lie{m}$.
Now, all ingredients are set up; we can say that if $\lie{g} = \lie{g}_1 \oplus \lie{g}_2$, then a $\lie{g}$-submodule $\lie{m}$ must be equal to $\lie{g}_1 \oplus \lie{g}_2$, a contradiction to the fact that $\lie{m}$ is proper.
Finally, we have proved that $\lie{g}$ is proper, which implies, as mentioned, the existence of an (unique) isomorphism of $\lie{g}_1$ onto $\lie{g}_2$ which sends $E_i$, $F_i$, $H_i$ to $E'_i$, $F'_i$, $H'_i$, respectively.

\newpage

\textbf{Isomorphism theorem: A more concrete way}

We have proved the isomorphism theorem in a beautiful way.
But for some purpose we need to construct an isomorphism in more concrete way.

The first step is to show a lemma: \textit{For any tuple of indices $i_1, i_2, \cdots, i_r$ with $[E_{i_r} E_{i_{r - 1}} \cdots E_{i_2} E_{i_1}] \ne 0$ and $\sigma \in S_r$ there is a rational $c_{(i_j), \sigma}$ such that $[E_{i_{\sigma(r)}} E_{i_{\sigma(r - 1)}} \cdots E_{i_{\sigma(2)}} E_{i_{\sigma(1)}}] = c_{(i_j), \sigma} [E_{i_r} E_{i_{r - 1}} \cdots E_{i_2} E_{i_1}]$, which is determined only by what $\sigma$ and $(i_j)$ and the Cartan matrix are}.
Now we assume that it holds for some $r - 1$ if we have $i_1, i_2, \cdots, i_{r - 1}$ and $\sigma \in S_{r - 1}$.
The idea is to apply $( \ad{E_p} ) ( \ad{F_p} )$, $p = i_r$, to $[(E_{i_{\sigma(j)}})]$.
As known, $\ad{F_p}$ acts on $[(E_{i_{\sigma(j)}})]$ as like this; replacing one of $E_p$ in the Lie product by $H_p$ and summing them up.
Also, $H_p$ can be replaced by some \textit{integer}, obviously, which depends only on what $(i_j)$ and $\sigma$ (and the Cartan matrix) are.
In other words, $( \ad{E_p} ) ( \ad{F_p} ) [(E_{i_{\sigma(j)}})]$ can be written as $\sum_{\sigma' \in S_{r - 1}} a_{(i_j), \sigma'} [E_{i_r} E_{i_{\sigma'(r - 1)}} E_{i_{\sigma'(r - 2)}} \cdots ]$, where $a_{(i, j), \sigma'}$ is the integer obtained from $H_p$ (or 0 if the replacement does not correspond to $\sigma'$).
Now by the assumption, $[E_{i_{\sigma'(r - 1)}} E_{i_{\sigma'(r - 2)}} \cdots ] = c_{(i_1, \cdots, i_{r - 1}), \sigma'} [E_{i_{r-1}} E_{i_{r-2}} \cdots]$, where $c_{(i_1, \cdots, i_{r - 1}), \sigma'}$ is rational and determined only by what $i_1, \cdots, i_{r - 1}$ and $\sigma'$ (and the Cartan matrix) are.
Thus, we have that $( \ad{E_p} ) ( \ad{F_p} ) [(E_{i_{\sigma(j)}})] = \left( \sum_{\sigma' \in S_{r - 1}} a_{(i_j), \sigma'} c_{(i_1, \cdots, i_{r - 1}), \sigma'} \right) [E_{i_r} E_{i_{r-1}} \cdots E_{i_2} E_{i_1}]$.

On the other hand, we denote $\beta = \sum_{j} \alpha_{i_j}$, which is of course a root.
Note that $[(E_{i_{\sigma(j)}})]$ can be regarded as a weight vector of a $\lie{g}_{\alpha_p}$-submodule, which corresponds to the $\alpha_p$-string through $\beta$.
Then $( \ad{E_p} ) ( \ad{F_p} )[(E_{i_{\sigma(j)}})] = (r_{(i_j), \sigma} + 1) (s_{(i_j), \sigma} + 1) [(E_{i_{\sigma(j)}})]$, where $-r_{(i_j), \sigma}$ and $s_{(i_j), \sigma}$ are the minimal and maximal value of $j$ with roots $\beta + j\alpha_p$.
But it could be 0, only if $\beta - \alpha_p$ is not a root, since in this case $(\ad{F_p}) \lie{g}_{\beta} = 0$.
Fortunately, since $[E_{i_{r - 1}} E_{i_{r - 2}} \cdots E_{i_2} E_{i_1}] \ne 0$, $\beta - \alpha_p$ is also a root.
Finally, we have that $[(E_{i_{\sigma(j)}})] = \frac{1}{(r_{(i_j), \sigma} + 1) (s_{(i_j), \sigma} + 1)} \times ( \ad{E_p} ) ( \ad{F_p} ) [(E_{i_{\sigma(j)}})] = \frac{\sum_{\sigma' \in S_{r - 1}} a_{(i_j), \sigma'} c_{(i_1, \cdots, i_{r - 1}), \sigma'}}{(r_{(i_j), \sigma} + 1) (s_{(i_j), \sigma} + 1)} [E_{i_r} E_{i_{r-1}} \cdots E_{i_2} E_{i_1}]$.
In conclusion, by the indcution on $r$, the proof can be done.
Note that the factor is a rational number and determined by what $i_1, \cdots, i_{r - 1}$ and $\sigma'$ (and the Cartan matrix) are.

With this lemma, one can find the multiplication table of the given simple Lie algebra.
For any root $\alpha = \sum_{j = 1}^r \alpha_{i_j}$, we choose $E_\alpha$ as $[E_{i_{r}} E_{i_{r - 1}} \cdots E_{i_{2}} E_{i_{1}}]$.
Consider arbitrary roots $\alpha, \beta$.
Then $[E_\alpha, E_\beta]$ could be written as a linear combination of $[E_{i_{\sigma(r)}} E_{i_{\sigma(r - 1)}} \cdots E_{i_{\sigma(2)}} E_{i_{\sigma(1)}}]$ ($\sigma \in S_r$), where $\alpha + \beta = \sum \alpha_{i_j}$.
Combining this with the lemma, we can argue that $[E_\alpha, E_\beta] = c_{\alpha, \beta} [E_{i_{r}} E_{i_{r - 1}} \cdots E_{i_{2}} E_{i_{1}}]$, where $c_{\alpha, \beta}$ is rational and determined only by what $\alpha, \beta$ are.
Also one can obtain similar results on $[F_\alpha, F_\beta]$ and $[E_\alpha, F_\beta]$.
Now, with this one can set an isomorphism between $\lie{g}_1$ and $\lie{g}_2$ in the previous page; a linear map $\phi$ which sends $E_\alpha = [E_{i_{r}} E_{i_{r - 1}} \cdots E_{i_2} E_{i_1}]$ to $E_{\alpha'} = [E_{i_{r}}' E_{i_{r - 1}}' \cdots E_{i_2}' E_{i_1}']$ and $F_\alpha = [F_{i_{r}} F_{i_{r - 1}} \cdots F_{i_2} F_{i_1}]$ to $F_{\alpha'} = [F_{i_{r}}' F_{i_{r - 1}}' \cdots F_{i_2}' F_{i_1}']$, and $H_i$ to $H_i'$.
Then, we have that $[E_\alpha', E_\beta'] = c_{\alpha, \beta} E_{\alpha + \beta}$ so that $\phi([E_\alpha, E_\beta]) = c_{\alpha \beta} E_{\alpha + \beta}' = [E_\alpha', E_\beta'] = [\phi(E_\alpha), \phi(E_\beta)]$, and similar things hold for $[F_\alpha, F_\beta]$ and $[E_\alpha, F_\beta]$.
...
The main lemma of the (stronger) Ado's theorem is the following one: \textit{Assume that $\lie{g}$ is solvable and $\mathcal{I} \trianglelefteq U(\lie{g})$ satisfies (i) $\mathcal{I}$ is finite-codimensional and (ii) for every $N \in \lie{n} = \Nil{\lie{g}}$ $N + \mathcal{I}$ is nilpotent.}
...
Remind a notation $\lie{g}_a(\tau) = \{ X \in \lie{g} \SBar (\tau - a1)^r X = 0 \textrm{ for some $r \in \NaN$} \}$ for any scalar $a$ and any $\tau \in L(\lie{g})$.
Let $\tau \in \textrm{Aut}(\lie{g})$.
It is easy to show that $\lie{g}_1(\tau)$ is a subalgebra of $\lie{g}$.
What we shall show is \textit{for any $\tau \in \textrm{Int}(\lie{g})$ $\dim{\lie{g}_1(\tau)}$ is bigger than or equal to the rank of $\lie{g}$.}
First, we prove this for the simplest case; $\tau = \exp{\ad{Z}}$ for a nilpotent $Z \in \lie{g}$.
If one can show that $\dim{\lie{g}_0(\ad{Z})} \ge l$, $l$ the rank, then $\dim{\lie{g}_1(\tau)} \ge l$, obviously.
But we already have the inequality because $\dim{\lie{g}_0(\ad{Z})}$ is an Engel subalgebra and every minimal Engel subalgebra, which is a CSA, has the dimension $l$.

Now we handle the general case: $\tau = \exp{\ad{X_1}} \exp{\ad{X_2}} \cdots \exp{\ad{X_r}}$ with all $\ad{X_i}$ nilpotent.
We shall use $\tau(t) = \prod_{i = 1}^r \exp{\ad{t_i X_i}}$, which is in $\textrm{Aut}(\lie{g}_K)$, $K$ the field of quotients of $k[[t_1, t_2, \cdots, t_r]]$.
Remind the application of CBH formula to a Lie algebra over such $K$, then one can find $Z(t) \in \lie{g}_K$ such that $\tau(t) = \exp{\ad{Z(t)}}$.
On the other hand, since $\lie{g}_K$ is semisimple with rank $l$ ($\kappa_\lie{g}$ is non-degenerate and the entries of the matrix representation of $\kappa_\lie{g}$ relative to the basis $(E_\alpha, H_i)$ are all rational), $\dim{(\lie{g}_K)_0(\ad{Z(t)})} \ge l$, so $\dim{(\lie{g}_K)_1(\tau(t))} \ge l$.
Let's consider the characteristic polynominal $c_{\tau(t)}(s) = s^{n} + \sum_{j = 0}^{n - 1} f_i(t) s^j$.
Note that for any evaluation $t_i \mapsto a_i$ the characteristic polynominal of $\tau(a_1, a_2, \cdots, a_r)$ is equal to $s^{n} + \sum_{j = 0}^{n - 1} f_i(a) s^j$.
Also, the inequality $\dim{(\lie{g}_K)_1(\tau(t))} \ge l$ says that $f_j(t) = 0$ if $j < l$.
Then $f_j(a) = 0$ for any $(a_i)$ if $j < l$, which implies that $\dim{\lie{g}_1(\prod_{i = 1}^r \exp{\ad{a_i X_i}})} = \dim{\lie{g}_1(\tau(a))} \ge l$ for any $a_i$'s in $k$.\footnote{It can be proved in a different way. There is $m$ such that $(\tau(t) - 1)^{m - 1} (\lie{g}_K)_1(\tau(t)) \ne 0$ but $(\tau(t) - 1)^{m} (\lie{g}_K)_1(\tau(t)) = 0$. With respect to a certain basis of $\lie{g}$ the matrix representation of $( \tau(t) - 1 )^m$ can be written as $T(t) \left( \prod_{r} S_r(t) \right)$, where $S_r(t)$ is a simple (column-)permutation matrix or $1 + g_i(t) E_{i(r)j(r)}$ for some (simple) power series and $T(t)$ has $s = \dim{(\lie{g}_K)_1(\tau(t))}$ null columns. Then $( \tau(a) - 1 )^m$ has a matrix representation as $T(a) \left( \prod_r S_r(a) \right)$, and $T(a)$ has at least $s$ null columns, which implies that $\dim{\ker{(\tau(a) - 1)^m}} \ge m \ge l$. Since $\ker{(\tau(a) - 1)^m}$ is contained in $\lie{g}_1(\tau(a))$, we have that $\dim{\lie{g}_1(\tau(a))} \ge l$.}
Finally, we have that $\dim{\lie{g}_1(\tau)} \ge l$ for every $\tau \in \textrm{Int}(\lie{g})$.
...
We call an automorphism of this type a \textbf{graph automorphism}, and denote the set (subgroup) of graph automorphisms by $\Gamma(\lie{g})$, where the graph automorphisms are well-defined by the isomorphism theorem (sending $E_i \mapsto E_{\tau(i)}$ and $F_i \mapsto F_{\tau(i)}$).
...
To show this we shall show that \textit{if $\sigma \in \textrm{Int}(\lie{g})$ preserves $\lie{h}$ and satisfies that $(\sigma|_\lie{h})^* \alpha = \alpha$ for any $\alpha$ in a given base $\Delta$, then for any $H \in \lie{h}$ $\sigma(H) = H$} (thus $\sigma \in E(\lie{g})$), which implies that \textit{every non-trivial graph automorphism is not inner}.
...
By using this fact we shall shrink $\lie{g}_1(\sigma)$.

Let $\sigma' = \tau \sigma$, where $\tau \in E(\lie{g})$ sends $E_i$ to $c_i E_i$ fixes $H_i$ for every $i$, for some non-zero scalars $c_i$.
First, we need a simple lemma: for any positive root $\alpha = \sum \alpha_{i_j}$, $\tau(E_{\pm \alpha}) = \left( \prod a_{i_j}^{\pm 1} \right) E_{\pm \alpha}$, which is immediate.
We want to make $\lie{g}_1(\sigma') \cap \lie{g}_{\pm r} = 0$ for any $r$ since $\lie{g} = \lie{h} \oplus \bigoplus_r (\lie{g}_r \oplus \lie{g}_{-r})$.
...
If we choose $c_i$'s to make the constant term not equal to 1 for all $r$, which is always possible, then $t - 1$ does not divide the polynomial so that $\lie{g}_1(\sigma') \cap \lie{g}_{\pm r} = 0$, which we want.
Then $\lie{g}_1(\sigma) \subseteq \lie{h}$, but since $\dim{\lie{g}_1(\sigma)} \ge \dim{\lie{h}}$, $\lie{g}_1(\sigma) = \lie{h}$.
Finally, by the assumption of the permutational natural of $\sigma$ (so that $(\sigma|_\lie{h})^r = 1$ for some $r$), $\sigma|_\lie{h}$ is semi-simple, which implies that $\lie{g}_1(\sigma) = \ker{(\sigma - 1)}$, so that $\sigma(H) = H$ for every $H \in \lie{h}$.
...
There is an important note: There is no restriction on a choice of distinct $c_j$, so that one can set them as (nonnegative) integers.
Hence, one can argue that all $a_{ij}$ can be (nonnegative) integers.
...
Let $\rho$ be a finite-dimensional irreducible representation of $\lie{g}$.
We call $\lie{g} \ni X \mapsto \tr{(\rho(X)^r)}$ ($r \in \NaN$) a \textbf{trace polynomial}, which is obviously in $\mathfrak{B}(\lie{g})$ since this is a polynomial of entries of $\rho(X)$ (relative to a basis).
We wonder how $G$ acts on this function.
For $\tau\in G$ we define a new representation $\rho^\tau$ as $\rho^\tau(X) = \rho(\tau(X))$.
Let $Z \in \mathcal{N}(\lie{g})$.
Then one can find a BSA which contains $Z$, and from this one can obtain a CSA $\lie{h}$ and a base $\Delta$ relative to $\lie{h}$ such that $Z \in \lie{n}(\Delta)$.
Let $\lambda$ be the highest weight of $V$ relative to $\Delta$ and $v^+$ the maximal vector of $V$.
Then for $H \in \lie{h}$, since $\tau(H) - H \in \lie{n}(\Delta)$, it is immediate that $\rho^\tau(H) v^+ = \rho(H)v^+$.
Furthermore, since $\tau(\lie{n}(\Delta)) \subseteq \lie{n}(\Delta)$, for all $N \in \lie{n}(\Delta)$ $\rho^\tau(N) v^+ = 0$, so $v^+$ is also the maximal vector of the representation $\rho^\tau$ with highest weight $\lambda$.
Thus, the representations $\rho$ and $\rho^\tau$ are equivalent so that there is a module isomorphism $\psi_\tau : V \to V$ which satisfies that $\rho(X) (\psi_\tau v) = \psi_\tau(\rho^\tau(X) v)$.
(N.B., this is not necessarily a scalar multiplication since $\rho$ and $\rho^\tau$ are different representations.)
From this we obtain that for any $X \in \lie{g}$ $\rho(X)$ and $\rho^\tau(X)$ are similar.
Hence, $\tr{(\rho^\tau(X)^r)} = \tr{(\rho(X)^r)}$.
But $\tau$ sends the polynomial function $X \mapsto \tr{(\rho(X)^r)}$ to $X \mapsto \tr{(\rho(\tau(X))^r)} = \tr{(\rho^\tau(X)^r)}$.
Therefore, since such $\tau$ generates $G$, we have that \textit{the trace polynomial is in $\mathfrak{B}(\lie{g})^G$}.
...
To show this (Steinberg), it is sufficient to show that all $\textrm{Sym}\; \lambda^r$ for all $\lambda \in \Lambda^+$ lies in the image of $\theta$.
Let $\rho_\lambda$ be the irreducible representation of $\lie{g}$ with highest weight $\lambda$, and we denote the trace polynomial $\lie{h} \ni H \mapsto \tr{(\rho_\lambda(H))^r}$ by $\eta_\lambda^r$.
Then for any $\lambda \in \Lambda^+$ and $r \in \NaN$, $\eta_\lambda^r$ lies in the image of $\theta$.
Here is the main idea: We will write every $\textrm{Sym}\; \lambda^r$ as a linear combination of these trace polynomials.
By the fact that there is a basis of $V(\lambda)$ consisting of weight vectors, for $H \in \lie{h}$ we have that $\eta_\lambda^r = \sum_{\mu \in \Pi(\lambda)} m_\lambda(\mu) \mu(H)^r = (\textrm{Sym}\; \lambda^r)(H) + \sum_{\mu \in ( \Pi(\lambda) \setminus W\lambda ) \cap \Lambda^+} m(\mu) (\textrm{Sym}\; \mu^r)(H)$.
Now, one can argue that we are done by using an induction on $\#(\Pi(\lambda) \cap \Lambda^+)$.
...
All what we have seen then shows that $\theta|_\mathfrak{T}$ is bijective, which will be important.
...
Let $\lie{h}$ be a Cartan subalgebra of $\lie{g}$ and $\Phi$ the root system, and $Z \in U(\lie{g})$ fixed by $G$.
It is immediate that if one shows that $[E_\alpha, Z] = 0$ for every $E_\alpha \in \lie{g}_\alpha$ and $\alpha \in \Phi$, then $Z$ is in $\mathfrak{C}$, since all such $E_\alpha$ generates $\lie{g}$.
And it is easy to show since we have that $(\ad{E_\alpha}) Z = (\log{(\exp{\ad{E_\alpha}})}) Z = \sum_{r > 0} \frac{(-1)^{r + 1}}{r} ((\exp{\ad{E_\alpha}}) - 1)^r Z = 0$ (it makes sense since $\ad{E_\alpha}$ is locally nilpotent).
...
We need a property: \textit{For $\lambda \in \Lambda$ let $\alpha$ be a simple root with positive $m = \langle \lambda, \alpha \rangle$.
Then the coset of $Y_\alpha^{m + 1}$ in $U(\lambda)$ is a submodule which is standard cyclic having highest weight $\lambda - (m + 1)\alpha$}; it is immediate from the fact that for a maximal vector $v^+$ $E_i Y_\alpha^{m + 1} v^+ =0$ for all $i$, which is shown in the theorem for finite-dimensionalness of $V(\lambda)$.
...
We need one more property: \textit{For distinct $\lambda, \mu \in \Lambda$ and a simple root $\alpha$ if $\lambda + \delta = \sigma_\alpha (\mu + \delta)$, then at least one of $\langle \lambda, \alpha \rangle$ and $\langle \mu, \alpha \rangle$ is nonnegative.}
Assume the $\langle \lambda, \alpha \rangle$ is negative.
Then $\langle \mu, \alpha \rangle = \langle \sigma_\alpha (\lambda + \delta) - \delta, \alpha \rangle = -\langle \lambda, \alpha \rangle - 2$, which is bigger than or equal to -1.
But if it is equal to $-1$, then $\langle \sigma_\alpha (\lambda + \delta), \alpha \rangle = 0$, which implies that $\lambda = \mu$.
Hence, we have that $\langle \mu, \alpha \rangle$ is nonnegative, as desired.
Assembling the facts we have found, we finally obtain that $\chi_\lambda \sim \chi_\mu$.

This result is most of a half part of a theorem we shall prove.
In fact, the result can be extended; \textit{for $\lambda, \mu \in \lie{h}^*$ if $\lambda \sim \mu$, then $\chi_\lambda = \chi_\mu$.}, as we will see.
...
We can consider a linear mapping $\xi_0 : U(\lie{g}) \to U(\lie{h})$ which fixes elements $\prod H_i^{q_i}$ and sends others in the PBW basis to 0, and we denote $\xi_0|_\mathfrak{C}$ by $\xi$.
If we confuse $\lambda$ with the $k$-algebra homomorphism of $U(\lie{h})$ into $k$ generated by $\lambda$, we have that $\chi_\lambda = \lambda \circ \xi$.
Note that since $\chi_\lambda$ and $\lambda$ are homomorphisms, $\xi(Z_1) \xi(Z_2) - \xi(Z_1 Z_2)$ are in $\bigcap_{\lambda \in \lie{h}^*} \ker{\chi_\lambda}$ for every $Z_1, Z_2 \in \mathfrak{C}$, which implies that $\xi$ is a homomorphism (although $\xi_0$ is not in general).
...
It is immediate that $\mathfrak{B}(\lie{g})$, or the symmetric polynomial algebra of $\lie{g}$ is isomorphic to the subalgebra of symmetric tensors $\tilde{\mathcal{S}}(\lie{g}) \subseteq T(\lie{g})$, and that by one of corollaries of PBW theorem $\tilde{\mathcal{S}}(\lie{g})$ is complementary to the kernel $I_L$ of the canonical mapping $T(\lie{g}) \to U(\lie{g})$.
It implies that the canonical mapping yields a bijective linear map $\pi' : \mathfrak{B}(\lie{g}^*) \to (\tilde{\mathcal{S}}(\lie{g}) + I_L) / I_L$.
Even by considering PBW basis one can find that $(\tilde{\mathcal{S}}(\lie{g}) + I_L) / I_L = U(\lie{g})$.
Moreover, since $\tilde{\mathcal{S}}(\lie{g})$ and $I_L$ are $G$-invariant, it is obvious that $\pi'$ is a $G$-module homomorphism (but not an algebra homomorphism).
Thus, we have a $G$-module isomorphism $\pi = \pi'|_{\mathfrak{B}(\lie{g}^*)^G} : \mathfrak{B}(\lie{g}^*)^G \to U(\lie{g})^G = \mathfrak{C}$.

Unfortunately, $\tau_\lie{g}$ and $\tau_\lie{h}$ cannot lift $\theta$ to $\psi \circ \pi$.
It means that for some $X \in \mathfrak{B}(\lie{g^*})^G$ $((\psi \circ \pi)(X))'$ does not coincide with $\theta(X')$.
...
\textbf{Preliminary result to the main formula for characters}

We need one more formula, which comes from Harish-Chandra's theorem.
We shall use this.
For $\lambda \in \lie{h}^*$ let $\mathfrak{M}_\lambda$ be the set of $\lie{g}$-modules $V$ which satisfy that (1) $V$ is the direct summation of finite-dimensional weight spaces (2) the action of $C \in \mathfrak{C}$ on $V$ is just by $\chi_\lambda(C)$ (3) all weight of $V$ is the finite union of some $F(\mu)$, where $F(\mu) = \{\mu - \sum_{\alpha \succeq 0} a_\alpha \alpha \SBar a_\alpha \in \NaN_0\}$.
It is obvious that $\mathfrak{M}_\lambda$ contains all standard cyclic module of $\lambda$, and all of its submodules and quotient modules.
Note that by Harish-Chandra's theorem $\mathfrak{M}_\lambda = \mathfrak{M}_\mu$ if $\lambda \sim \mu$, and also if a standard cyclic module of $\mu \in \lie{h}^*$ is in $\mathfrak{M}_\lambda$, then $\lambda \sim \mu$.

Now we introduce a lemma: \textit{any $0 \ne V \in \mathfrak{M}_\lambda$ has at least one maximal vector}, which is almost obvious if (3) is considered (Hint: One can choose a minimal $M$ of weights of $V$ such that $\bigcup_{\mu \in M} F(M)$ contains all weights of $V$).
We let $\theta(\mu) = \{\nu \in \lie{h}^* \SBar \mu \sim \nu, \nu \prec \mu\}$ for $\mu \in \lie{h}^*$.
Now we shall prove that \textit{$Z(\lambda)$ has a composition series, that is, submodules $Z_i$ such that $Z(\lambda) = Z_0 \gneq Z_1 \gneq Z_2 \gneq \cdots \gneq Z_n = 0$ such that for each $i$ $Z_i / Z_{i + 1} = V(\mu_i)$ ($\mu_i \in \theta(\lambda)$).}
To obtain the finite series, we need to give a certain integer variable to each module in $\mathfrak{M}_\lambda$; $l(V) = \sum_{\mu \in \theta(\lambda)} \dim{V_\mu}$, which is well-defined by (1).
Now, if one chooses any non-irreducible standard cyclic $V \in \mathfrak{M}_\lambda$ of $\mu \in \lie{h}^*$ and find a standard cyclic submodule $V'$ of $\mu'$ such that $\mu' \in \theta(\mu)$ and $l(V'), l(V/V') < l(V)$, then the lemma is obtained immediately.
One can choose a submodule of $V$ and, as mentioned, a maximal vector in the submodule, which is denoted by $v'^+$, of weight $\mu'$.
We also denote $V' = U(\lie{g})v'^+$.
Note that by Harish-Chandra's theorem $\mu' \in \theta(\mu)$.\footnote{If $\mu$ is not in $\Lambda$, there would be no such $\mu'$, in other words, $\theta(\mu) = \{\mu\}$.}
Now, note that $\dim{V'_\nu}, \dim{(V/V')_\nu} \le \dim{V_\nu}$ for any $\nu \in \theta(\mu)$.
But $V'_\mu = 0$ while $V_\mu \ne 0$, and also $\dim{(V/V')_{\mu'}} \le \dim{V_{\mu'}} - 1$ since $v'^+ + V' = 0$.
Therefore, $l(V'), l(V/V') < l(V)$, which finishes the proof of the lemma so that we have the promised composition series for $Z(\lambda)$.

Now, we define an important mapping for $V \in \mathfrak{M}_\lambda$.
Since all weight spaces of $V$ is finite-dimensional, one can define a map $ch_{V} : \lie{h}^* \to \InZ$, defined as $ch_V(\mu) = \dim{V_\mu}$ for $\mu \in \lie{h}^*$, where this kind of maps will be treated more systematically in the next page.
These results gives a key stone; $ch_{Z(\lambda)} = ch_{V(\lambda)} + \sum_{\lambda \ne \mu \in \theta(\lambda)} d(\mu) ch_{V(\mu)}$ with $d(\mu) \in \NaN$ (note that $\mu_i \ne \lambda$ if $i \ne 0$).
To obtain a formula for $ch_{V(\lambda)}$, we assume that there is $m \in \NaN$ such that for every $\lambda \in \lie{h}^*$ with $|\theta(\lambda)| < m$ $ch_{V(\lambda)}$ can be written as $\sum_{\mu \in \theta(\lambda)} c(\mu) ch_{Z(\lambda)}$ with integers $c(\mu)$, $c(\lambda) = 1$, and now assume that $|\theta(\lambda)| = m$.
Note that for any $\lambda \ne \mu \in \theta(\lambda)$ we have that $\theta(\mu) \subsetneq \theta(\lambda)$, so that $|\theta(\mu)|< |\theta(\lambda)| = m$.
Thus, by the assumption, $ch_{V(\lambda)} = ch_{Z(\lambda)} - \sum_{\lambda \ne \mu \in \theta(\lambda)} d(\mu) ch_{V(\mu)} = \sum_{\lambda \ne \mu \in \theta(\lambda)} d(\mu) \sum_{\mu \ne \nu \in \theta(\mu)} c_\mu(\nu) ch_{Z(\nu)}$ for some integers $c_\mu(\nu)$.
Then, by the induction on $m$ (if $|\theta(\lambda)| = 1$, $ch_{Z(\lambda)} = ch_{V(\lambda)}$), we have that \textit{$ch_{V(\lambda)} = \sum_{\mu \in \theta(\lambda)} c(\mu) ch_{Z(\lambda)}$ with integers $c(\mu)$, $c(\lambda) = 1$}.

There are some notable things: For any non-negative integers $a_i$, by considering PBW-basis, we have that $\dim{Z(\lambda)_{\lambda - \sum a_i \alpha_i}} = \dim{Z(0)_{-\sum a_i \alpha_i}}$, which implies that $ch_{Z(\lambda)}(\mu) = ch_{Z(0)}(\mu - \lambda)$.
Also, if $\lambda \in \Lambda^*$, then $\theta(\lambda) = \{ \sigma (\lambda + \delta) - \delta \SBar \sigma \in W \}$ and $|\theta(\lambda)| = |W|$ since $\lambda + \delta$ is strongly dominant.
Thus, we can write $ch_{V(\lambda)} = \sum_{\sigma \in W} c(\sigma) ch_{Z(\sigma(\lambda + \delta) - \delta)}$.

\newpage

\textbf{From formal character to functions on $\lie{h}^*$, Kostant's formula}

Now to we look up to the formal characters, we need to make formal characters more abtract; a mapping of $\Lambda$ to $Z$.
We have a $\InZ$-module structure on the set of maps of $\lie{h}^*$ into $\InZ$, $\InZ[\lie{h}^*]$.
We want to give a multiplication structure from \textbf{convolution}, $(f * g)(\lambda) = \sum_{\mu_1 + \mu_2 = \lambda} f(\mu_1) g(\mu_2)$, but it does not make sense for arbitrary $f, g \in \InZ[\lie{h}^*]$.
To use this and to include $ch_{Z(\lambda)}$, let $\mathfrak{X}_F = \left\{ f \in \InZ[\lie{h}^*] \SBar \textrm{supp} \; f \subseteq \bigcup_{\mu \in M} F(\mu), \; |M| < \infty \right\}$, where $\textrm{supp} \; f = \{\mu \in \lie{h}^* \SBar f(\mu) \ne 0\}$.
It's easy to check that for every $f, g \in \mathfrak{X}_F$ and $\lambda \in \lie{h}^*$ the number of pairs $(\mu_1, \mu_2) \in \textrm{supp} \; f \times \textrm{supp} \; g$ with $\mu_1 + \mu_2 = \lambda$ is finite, which implies that $\mathfrak{X}_F$ with the convolution has a commutative ring structure.
Especially, we denote $\mathfrak{X} = \{ f \in \mathfrak{X}_F \SBar |\textrm{supp} \; f| < \infty\}$, which is obviously a subring.

To study about $ch_\lambda \equiv ch_{V(\lambda)}$ for $\lambda \in \Lambda^*$, which is in $\mathfrak{X}$ obviously, we need some tools.
The \textbf{characteristic function} $\epsilon_{\lambda}$, which is defined as $\epsilon_{\lambda}(\lambda) = 1$ while $\epsilon_{\lambda}(\mu) = 0$ for any $\lambda \ne \mu \in \lie{h}^*$ plays an important role, which corresponds to $e(\lambda)$.
They are immediate that for every $\lambda, \mu \in \lie{h}^*$ and $f, g \in \mathfrak{X}_F$ (1) $\epsilon_\lambda * \epsilon_\mu = \epsilon_{\lambda + \mu}$, (2) $(f * \epsilon_\lambda)(\mu) = f(\mu - \lambda)$, (3) $f = \sum_{\mu \in \textrm{supp} \; f} f(\mu) \epsilon_\mu$, especially $f * g = \sum_{\mu_1, \mu_2} f(\mu_1) g(\mu_2) \epsilon_{\lambda + \mu}$.
The last property implies that for a $\InZ$-module homomorphism $\phi : \mathfrak{X}_F \to \mathcal{R}$, $\mathcal{R}$ a ring, if $\phi(\epsilon_\lambda * \epsilon_\mu) = \phi(\epsilon_\lambda) * \phi(\epsilon_\mu)$, then $\phi$ is a ring-homomorphism.
Also, it implies that for a $\InZ$-module homomorphism $\phi : \mathfrak{X}_F \to \mathfrak{X}_F$, if $\phi(\epsilon_\lambda * \epsilon_\mu) = \phi(\epsilon_\lambda) * \epsilon_\mu + \epsilon_\lambda * \phi(\epsilon_\mu)$, then $\phi$ is a derivative.
For example, for each $\sigma \in W$, we extend this to a map $\sigma : \mathfrak{X}_F \to \mathfrak{X}_F$, which satisfies that $(\sigma f)(\mu) = f(\sigma^{-1} \mu)$ for any $f \in \mathfrak{X}_F$ and $\mu \in \lie{h}^*$.
Then first $\sigma \epsilon_\lambda = \epsilon_{\sigma \lambda}$, so that $\sigma (\epsilon_{\lambda + \mu}) = \epsilon_{\sigma \lambda} * \epsilon_{\sigma \mu}$, which implies that $\sigma$ is a ring-homomorphism.

Now, we need to investigate $ch_{Z(\lambda)}$.
The fact that $ch_{Z(\lambda)}(\mu) = ch_{Z(0)}(\mu - \lambda)$ implies that $ch_{Z(\lambda)} = p * \epsilon_\lambda$ where $p = ch_{Z(0)}$.
By counting PBW-basis, it is immediate that $p(\mu) = |\{ (a_\alpha) \SBar a_\alpha \in \NaN_0, \sum_{\alpha \succ 0} a_\alpha \alpha = -\mu \}|$, which implies that $p(\mu) = \sum_{(a_\alpha)} \epsilon_{-\sum a_\alpha \alpha} (\mu)$, or $p = \prod_{\alpha \succ 0} f_\alpha$, where $f_\alpha = \sum_{r = 0}^{\infty} \epsilon_{-r\alpha}$.
It is immediate that $(1 - \epsilon_{-\lambda}) f_\alpha = 1$, so if we define $q = \prod_{\alpha \succ 0} \left( \epsilon_{\alpha/2} - \epsilon_{-\alpha/2} \right)$, which is equal to $\epsilon_\delta \prod_{\alpha \succ 0} \left( 1 - \epsilon_{-\alpha} \right)$, we have that $q * p = \epsilon_\delta$.
With this, we can investigate the main result in the previous page: $ch_\lambda  = \sum_{\sigma \in W} c(\sigma) ch_{Z(\sigma(\lambda + \delta) - \delta)}$.
The RHS can be written as $p * \sum_{\sigma \in W} c(\sigma) \epsilon_{\sigma(\lambda + \delta) - \delta}$.
Thus, we have that $q * ch_\lambda = \sum_{\sigma \in W} c(\sigma) \epsilon_{\sigma(\lambda + \delta)}$.
Now, for $\sigma \in W$ we have that $\sigma(q * ch_\lambda) = \sigma(q) * \sigma(ch_\lambda) = \sigma(q) * ch_\lambda = \sum_{\sigma' \in W} c(\sigma^{-1} \sigma') \epsilon_{\sigma'(\lambda + \delta)}$.
On the other hand, we have that $\sigma_i q = \left( \epsilon_{-\alpha_i/2} - \epsilon_{\alpha_i/2} \right) \prod_{\alpha \succ 0, \alpha \ne \alpha_i} \left( \epsilon_{\alpha/2} - \epsilon_{-\alpha/2} \right) = -q$, since $\sigma_i$ permutes all positive roots except $\alpha_i$, which implies that $\sigma q = sn(\sigma) q$.
Thus, $q * ch_\lambda = sn(\sigma) \sum_{\sigma' \in W} c(\sigma^{-1} \sigma') \epsilon_{\sigma'(\lambda + \delta)}$ for every $\sigma$, which implies that $c(\sigma) = sn(\sigma)$.
Finally, we have obtained that (*) $q * ch_\lambda = \sum_{\sigma \in W} sn(\sigma) \epsilon_{\sigma(\lambda + \delta)}$.

Now, it is time to show one important result.
Multiplying (*) by $\epsilon_{-\delta} * p$, we have that $m_\lambda(\mu) = \sum_{\sigma \in W} sn(\sigma) p(\mu + \delta - \sigma(\lambda + \delta))$, the \textbf{Kostant formula}.
...
Maybe a $\InZ$-module homomorphism $v : \mathfrak{X} \to \InZ$ with $v(\epsilon_\mu) = 1$ for all $\mu$ satisfies that, and surprisingly, since $v(\epsilon_{\mu_1 + \mu_2}) = 1 = v(\epsilon_{\mu_1}) v(\epsilon_{\mu_2})$, it is even a ring-homomorphism.
...
To solve this we shall see that for a root $\alpha$ $\epsilon_\lambda \mapsto (\lambda, \alpha) \epsilon_\lambda$ extends to an endomorphism $\partial_\alpha$ of $\mathfrak{X}_0$ which is actually a derivative (see $\partial_\alpha (\epsilon_{\mu_1 + \mu_2})$), and let $\partial = \prod_{\alpha \succeq 0} \partial_\alpha$.
...
Thus we have that $v(\partial(\omega(\delta) * ch_\lambda)) = v(\partial(\omega(\delta))) deg(\lambda) = v(\partial(\omega(\lambda + \delta)))$ and $v(\partial(\omega(\delta)))$ would be nonzero, so we can get $deg(\lambda) = \frac{v(\partial(\omega(\lambda + \delta)))}{v(\partial(\omega(\delta)))}$.

Remind that $\omega(\lambda + \delta) = \sum_{\sigma \in W} sn(\sigma) \epsilon_{\sigma(\lambda + \delta)}$.
On the other hand, $\partial(\epsilon_{\sigma(\lambda + \delta)}) = (\prod_{\alpha \succeq 0} (\alpha, \sigma(\lambda + \delta))) \epsilon_{\sigma(\lambda + \delta)} = (-1)^{n(\sigma)} (\prod_{\alpha \succeq 0} (\alpha, \lambda + \delta)) \epsilon_{\sigma(\lambda + \delta)}$, and we have that $n(\sigma) = l(\sigma)$.
...
Let $w_i = (\alpha_i, \alpha_i) / (\alpha_r, \alpha_r)$, ($r$ is fixed with minimal $(\alpha_r, \alpha_r)$).
Then, for $\alpha = \sum a_i^\alpha \alpha_i$, $(\sum m_i \lambda_i, \alpha) = \sum a_i^\alpha m_j (\lambda_j, \alpha_i) = \sum a_i^\alpha m_j \frac{(\lambda_j, \alpha_i)}{(\alpha_i, \alpha_i)} (\alpha_i, \alpha_i)$, so $(\sum m_i \lambda_i, \alpha)$ can be written as $\frac{(\alpha_r, \alpha_r)}{2} \sum a_i^\alpha w_i m_i$.
From this, $deg(\lambda) = \prod_{\alpha \succeq 0} \frac{\sum a_i^\alpha w_i (m_i + 1)}{\sum a_i^\alpha w_i}$.
Note that if $\lambda(H_i) \le \lambda'(H_i)$ for every $i$, then $deg(\lambda) \le deg(\lambda')$.

This is what we need to finish the classification theorem, as mentioned.
It can be calculated that in the case for $B_l$ the roots are $\alpha_i + \cdots + \alpha_j$ and $\alpha_i + \cdots + \alpha_j + 2\alpha_{j + 1} + \cdots + 2\alpha_l$ with $w_1 = \cdots = w_{l - 1} = 2$ and $w_l = 1$; the case for $C_l$ $\alpha_i + \cdots + \alpha_j$ and $\alpha_i + \cdots + \alpha_j + 2\alpha_{j + 1} + \cdots + 2\alpha_{l - 1} + \alpha_l$ and $2\alpha_{i} + \cdots + 2\alpha_{l - 1} + \alpha_l$ with $w_1 = \cdots = w_{l - 1} = 1$ and $w_l = 2$.
From this one can directly (although it is complicating a bit; we omit it) calculate that in the case for $B_l$ $deg(\lambda_r) = \binom{2l + 1}{r}$ ($r < l$) and $deg(\lambda_l) = 2^l$, while in $C_l$ $deg(\lambda_r) = \binom{2l + 1}{r} - \binom{2l + 1}{r - 1}$.
In each case, the minimums are $2l + 1$ and $2l$ if $l \ge 3$.
...
Now for each $r$ we define a mapping $d_r : C^r(\lie{g}; V) \to C^{r + 1}(\lie{g}; V)$ as
\begin{eqnarray*}
  & & (d_i f)(X_0, X_1, \cdots, X_r) = \sum_i (-1)^i \rho(X_i) f(X_0, X_1, \cdots, \hat{X_i}, \cdots, X_r) + \\
...
If there is no ambiguity, we denote frequently $d_i$ by just $d$; for example, we denote $d_{i + 1} \circ d_i$ by $d \circ d$, or $d^2$.
There is an important result: \textit{for any case $d^2 = 0$}, where the proof is straightforward but dizzy.\footnote{Hint for proof; if one expands $d_{i + 1} (d_i f)$, there are several terms, but terms with triple Lie product (such like $[X_0, [X_1, X_2]]$) in arguments of $f$ are cancelled by themselves and so is terms with two Lie products in arguments of $f$. It should be so because it must work when $\rho$ is trivial.}
From this we can say about \textbf{cohomology}.

Now, we call $f \in C^r(\lie{g}; V)$ an \textbf{$r$-cocycle} if $df = 0$, and call $g \in C^r(\lie{g}; V)$ ($r > 0$) an \textbf{$r$-coboundary} if there is $h \in C^{r - 1}(\lie{g}; V)$ such that $g = dh$.
...
Then by definition $Z^r(\lie{g}; V) = \ker{d_r}$ and $B^r(\lie{g}; V) = \textrm{Im } d_{r - 1}$ if $r > 0$.

Since $d^2 = 0$, we have that $B^r(\lie{g}; V) \le Z^r(\lie{g}; V)$.
...
Note that if $f : \lie{g} \to V$ is 1-cocycle, or $df(A, B) = \rho(A) f(B) - \rho(B) f(A) - f([A, B]) = 0$ for every $A, B \in \lie{g}$, and $H^1(\lie{g}; V) = 0$, that is, $Z^1(\lie{g}; V) = B^r(\lie{g}; V)$, then there is $v \in V$ such that if $g : \lie{g} \ni A \to v$, then $f = dg$, or $f(A) = \rho(A)v$.
These functions will be used in some important moments.\footnote{Actually Weyl's complete reducibility theorem and Levi's theorem can be proved by using this; see [Jacobson]. But we will not introduce them.}
...
Thus it suffices to show that for each $f \in Z^r(\lie{g}; V)$ ($r > 0$) we can find $g \in C^{r - 1}(\lie{g}; V)$ such that $f = dg$.
...
{\small
\begin{eqnarray*}
  0 &=& \sum_s U_s (df)(X_0, X_1, \cdots, X_{r - 1}, U'_s) \;\;\; (df = 0) \\
...
Now if $g(X_0, \cdots, X_{r - 1}) = (-1)^{r + 1} C^{-1} \sum_s U_s f(X_0, \cdots, X_{r - 1}, U'_s)$, then we have that $g \in C^{r - 1}(\lie{g}; V)$ and $f = dg$, which we seek.

\newpage

\textbf{Levi-Mal'cev theorem}

We had proven that every finite-dimensional Lie algebra admits Levi factor.
One may ask whether there is a relation between two Levi factors if there are two Levi factors.
Mal'cev's the following theorem (or called \textbf{Levi-Mal'cev thereom}, combining the Levi's theorem) gives an strong answer; \textit{for finite-dimensional Lie algebra $\lie{g}$ with two Levi factors $\lie{g}_1$ and $\lie{g}_2$ there is an automorphism $\tau$ generated by $\exp{\ad{Z}}$ for $Z \in \Nil{\lie{g}}$ such that $\lie{g}_2 = \tau \lie{g}_1$}.

To prove this, we first attack a special case; $\lie{r} = \Rad{\lie{g}}$ is Abelian.
We can define an action of $\lie{g}_1$ on $\lie{r}$ by $\lie{g}_1 \ni A \mapsto \ad{A}|_\lie{r}$, and it gives a well-defined representation of $\lie{g}_1$.
Let $P_2$ be the projection of $\lie{g}_1$ into $\lie{r}$ with a decomposition $\lie{g} = \lie{g}_2 \oplus \lie{r}$.
Then it is direct that for $A_1, A_2 \in \lie{g}_1$ with $A_i = S_i + R_i$ where $S_i \in \lie{g}_2$ and $R_i \in \lie{r}$ $(\ad{A_1}) P_2(A_2) - (\ad{A_2}) P_2(A_1) = [S_1, R_2] + [R_1, S_2]$, which coincides with $P_2([A_1, A_2])$ (note that $[R_1, R_2] = 0$ because $\lie{r}$ is Abelian).
Thus, $P_2$ is 1-cocycle, so there is $Z \in \lie{r}$ such that $P_2(A_1) = [A_1, Z]$ for every $A_1 \in \lie{g}_1$ by Whitehead's theorem.
Note that since $\lie{r}$ is an Abelian ideal $\exp{\ad{Z}} = 1 + \ad{Z}$.
Then, for every $A \in \lie{g}_1$ we have that $(\exp{\ad{Z}}) A = A + (\ad{Z})A = A - P_2(A) \in \lie{g}_2$, which implies $(\exp{\ad{Z}})\lie{g}_1 \subseteq \lie{g}_2$.
Since $\dim{\lie{g}_1} = \dim{\lie{g}_2}$, we have that $(\exp{\ad{Z}})\lie{g}_1 = \lie{g}_2$.

Now we deal with the general case.
There is a shortcut for the proof, the following lemma: \textit{if $\lie{g}_1 \subseteq \lie{g}_2 + \lie{n}^{(r)}$ for some $r$, where $\lie{n} = \Nil{\lie{g}}$, then there is $Z \in \lie{n}^{(r)}$ such that $(\exp{\ad{Z}}) \lie{g}_1 \subseteq \lie{g}_2 + \lie{n}^{(r + 1)}$ (the exponential is well-defined since $Z \in \lie{n}$)}.
To prove this, again we shall find and use an 1-cocycle.
In this moment for $S_1, S_2 \in \lie{g}_2$ and $R_1, R_2 \in \lie{n}^{(r)}$ $P_2([S_1 + R_1, S_2 + R_2]) = [S_1, R_2] + [R_1, S_2] + [R_1, R_2]$ ($A_i = S_i + R_i$), with an extra term $[R_1, R_2]$, so it seems not in the form of $\rho(A_1) P_2(A_2) - \rho(A_2) P_2(A_1)$.
But since $[R_1, R_2] \in \lie{n}^{(r + 1)}$, $\overline{P_2}([A_1, A_2]) \equiv P_2([A_1, A_2]) + \lie{n}^{(r + 1)} = [S_1, R_2] + [R_1, S_2] + \lie{n}^{(r + 1)}$, which seems the required form.
From this, we expect that the corresponding representation for the cohomology structure making $\overline{P_2}$ an 1-cocycle must be $\rho : \lie{g}_1 \ni A \mapsto \overline{\ad{P_1(A)}} \in L(\lie{n}^{(r)} / \lie{n}^{(r + 1)})$, where $P_1 \equiv 1 - P_2 : \lie{g} \to \lie{g}_2$.
Note that $P_1$ is actually a Lie algebra homomorphism and any of $\ad{\lie{g}}$ sends an ideal into the ideal itself, so $\rho$ is a well-defined representation of $\lie{g}_1$.
Thus, $\overline{P_2}$ becomes an 1-cocycle, so there is $\overline{Z} \in \lie{n}^{(r)} / \lie{n}^{(r + 1)}$ such that $\overline{P_2}(S) = (\overline{\ad{S}}) \overline{Z}$ for every $S \in \lie{g}_1$, or $P_2(S) + [Z, S] \in \lie{n}^{(r + 1)}$ for any $Z \in \overline{Z}$.
Thus, $S + [Z, S] = P_1(S) + P_2(S) + [Z, S] \in \lie{g}_2 + \lie{n}^{(r + 1)}$ for every $S \in \lie{g}_1$, which implies that $(\exp{\ad{Z}})S \in \lie{g}_2 + \lie{n}^{(r + 1)}$ since $(\ad{Z})^i A \in \lie{n}^{(r)}$ so that $(\ad{Z})^{i + 1} A \in \lie{n}^{(r + 1)}$ for any $A \in \lie{g}$ and $i > 0$.
Therefore, $(\exp{\ad{Z}}) \lie{g}_1 \subseteq \lie{g}_2 + \lie{n}^{(r + 1)}$.

Finally, we can prove Mal'cev theorem.
By the above theorem and the solvability of $\Nil{\lie{g}}$ for any $\lie{g}$ and an induction on $r$, the remaining is to show $\lie{g}_1 \subseteq \lie{g}_2 + \lie{n}$.
Note that for any $A_1, A_2 \in \lie{g}_1$, $P_2([A_1, A_2]) = [P_1(A_1), P_2(A_2)] + [P_2(A_1), P_1(A_2)] + [P_2(A_1), P_2(A_2)]$, so $P_2([A_1, A_2]) \in [\lie{g}, \Rad{\lie{g}}] \subseteq \Nil{\lie{g}}$.
Since $\lie{g}_1 = [\lie{g}_1, \lie{g}_1]$, we have that $\lie{g}_1 \subseteq \lie{g}_2 \oplus \lie{n}$, which implies the first stage of the induction.
Finally, by induction, we obtain that there is an automorphism $\tau$ generated by $\exp{\ad{Z}}$ where $Z \in \Nil{\lie{g}}$ such that $\tau \lie{g}_1 \subseteq \lie{g}_2$.
Since their dimensional is same, we finally obtain that $\tau \lie{g}_1 = \lie{g}_2$.
